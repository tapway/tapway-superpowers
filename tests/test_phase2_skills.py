...
def main() -> int:
    ...
    check(db_claude is not None and ("alembic" in db_claude.lower() or "migration" in db_claude.lower()),
    ...
    # W4 hardening: assert the runnable example uses the CURRENT schemathesis API.
    # schemathesis.from_uri was removed — it's schemathesis.openapi.from_url in v4.24+.
    check(ac_claude is not None and "schemathesis.openapi" in ac_claude,
          "example uses current schemathesis.openapi.from_url (not removed from_uri)")
    check(ac_claude is not None and "from_uri" not in ac_claude,
          "no references to removed schemathesis.from_uri API")
