...
**When to invoke:** When adopting this skills set in a project repo (installed via
`codex/install.sh`), after `setup-project`,
...
Install `pytest-cov`. The equivalent for a JS/TS backend is `c8` or `nyc` with a `check-coverage` threshold.
