...
name: tdd
description: >-
  TDD-first task execution — each task is split into a Test Writer phase (RED)
  and an Implementer phase (GREEN + REFACTOR), with the coordinator enforcing
  the failing-test gate between phases. This is the default way to implement
  anything on this project. Triggers include "implement", "implement the plan",
  "let's implement", "begin implementation", "write a test first", "new feature",
  "bug fix", "behavior change", "delegate to subagents", "spawn agents".
version: 1.2.0
...
metadata:
  hermes:
    tags: [tdd, testing, red-green-refactor, implementation, quality]
    related_skills: [writing-plans, test-driven-development, requesting-code-review, github-pr-workflow]
---

# Skill: TDD-Driven Development (Hermes port)
...
> **Hermes note:** Hermes doesn't have Claude Code's per-task "agents" with a coordinator gate built in, but it *does* have `delegate_task` (subagents) and the `test-driven-development` skill. This port adapts the protocol: you (the coordinator) run the RED gate yourself between phases, exactly as the Claude plugin's coordinator does. The structural guarantee — a test must fail before any production code is written — is preserved.
...
### Before Starting
- [ ] You are on a **feature branch**, not `main` — `git branch --show-current` must not return `main`
...
- [ ] Consider creating the branch in an isolated worktree if tasks are truly independent
...
2. **For framework-specific tasks** (new middleware, ORM patterns, auth flows, cloud SDK calls): fetch the official docs for the detected version via `web_search` / `web_extract` — not blogs, not StackOverflow, not training data.
...
Use Hermes `delegate_task` with a leaf worker. Provide exactly this context — no more:
...
The coordinator (you) reviews the Test Writer's output before proceeding:
...
Invoke the `requesting-code-review` skill (or apply its checklist):
...
## Model Selection (for delegate_task)
...
- ❌ Never push commits manually after implementation — always exit via the PR workflow

## Exceptions (require explicit approval)
- Auto-generated files (migrations, OpenAPI-derived types) — skip TDD subagents, edit directly
- Pure configuration changes — no test needed, edit directly
