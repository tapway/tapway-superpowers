...
description: >
  ...
  auto-release workflow (semver vX.Y.Z-stg/prod), verifies AGENTS.md exists
  with TARGET_BRANCH set, installs the Tapway skills, commits everything, and
  prints a checklist of remaining manual steps. Triggers include "setup project",
  "initialize project for the team", "project setup", "add the release workflow".
---

# Skill: Setup Project

**When to invoke:** First time the Tapway skills are adopted in a project repo, or when the session-start hook reports that `.github/workflows/release.yml` is missing.
...
4. Runs `codex/install.sh` to install the Tapway skills into this project
5. Installs the git `pre-commit` backstop (`hooks/pre-commit/git-pre-commit.sh`) into `.git/hooks/pre-commit`
6. Commits all files and pushes
7. Prints a manual-steps checklist (default branch, filling in AGENTS.md, branch protection, CODEOWNERS)
...
> created and no AI-provider API key secret is required.
...
ls codex/skills 2>/dev/null && echo "EXISTS" || echo "MISSING"
...
### Step 3 — Install the Tapway Skills into This Project

Run `codex/install.sh` to install the Tapway skills (and hooks) into this project:

```bash
bash codex/install.sh
```

Confirm the skills landed in the project's skills directory (e.g. `codex/skills/` or the
`.agents/skills/` target used by the installer) and that `.codex/hooks.json` is present.
Then verify `$tapway` (the umbrella skill) is available for the project's pipeline steps.

### Step 4 — Install the Git Pre-Commit Backstop
...
## Do Not
- [Things Codex should never do in this repo]
```
...
git commit -m "chore: add auto-release workflow, AGENTS.md, and Tapway skills"
git push
```

If on `staging` or `main` and a push guard hook (`.codex/hooks.json`) blocks the push, create a branch:
...
### Step 7 — Print Manual Steps Checklist
...
Automated:
  ...
  ✅ codex/install.sh — Tapway skills installed into this project
...
Manual steps still required:
  ...
  ☐ Fill in AGENTS.md (stack, commands, conventions)
      → Codex reads this every session to understand your project
