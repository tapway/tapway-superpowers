...
description: >-
  Methodical root-cause debugging (Hermes port of tapway-superpowers systematic-debugging skill).
version: 1.2.0
...
metadata:
  hermes:
    tags: ["debugging", "root-cause", "incidents"]
...
> - For code bugs with a clear repro: use Claude Code's built-in **`/bugfix`** skill — it enforces repro-first with a regression test and opens a PR automatically.
> - For production incidents needing a written root-cause report: use Claude Code's built-in **`/investigate`** skill — it runs parallel hypothesis agents, adversarially refutes each, and produces a cited report.
>
