...
metadata:
  hermes:
    tags: [quality-gates, coverage, typescript, strict, env, codeowners, branch-protection, ci, quality]
    related_skills: [setup-project, verification, tdd, e2e-playwright, requesting-code-review, pr]
---

# Skill: Quality Gates — Config-Layer Enforcement (Hermes port)
...
> **Hermes note:** This skill is agent-invoked (like `setup-project`) — it lays down
> config files and instructs the user on manual GitHub steps (branch protection). The
> enforcement itself is structural (CI + compiler), so it runs forever after the skill
> completes. No `delegate_task` needed — the work is deterministic config authoring.
