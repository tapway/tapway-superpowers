...
Everything in between is normal dev work in the repo (AGENTS.md carries the
...
**Registering the gbrain MCP server (Codex):** Register the gbrain MCP server in
the project's `.codex/config.toml` so Codex can call its tools directly:
...
Then use the `put_page` tool via Codex's built-in MCP support.
...
- **Static docs live in AGENTS.md**; only *product state* lives in
