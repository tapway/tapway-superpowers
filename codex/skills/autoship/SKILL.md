...
|---|---|---|
| **Standard** | "implement it with autopilot" / "autoship" | CI/CD handles deployment; Codex is on a developer machine |
| **Deploy** | "implement and deploy" / "autoship with deployment" | Codex is running directly on the staging server |
...
CONVENTIONS: [key items from AGENTS.md]
...
Invoke the `$repo-docs` skill — mandatory on every autoship run, no exceptions. `$repo-docs` updates only the sections affected by this branch's changes if docs already exist, or generates all docs from scratch if this is the first time. Commit any changes it makes:
...
- ❌ Never skip the doc update step — update or generate docs before invoking `$pr`
