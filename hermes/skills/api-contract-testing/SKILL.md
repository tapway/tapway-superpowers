...
metadata:
  hermes:
    tags: [api-contract, schemathesis, openapi, pact, testing, backend, contract]
    related_skills: [e2e-playwright, verification, tdd, quality-gates, requesting-code-review, pr]
---

# Skill: API Contract Testing (Hermes port)
...
> **Hermes note:** This skill is coordinator-driven (like `e2e-playwright`). The
> agent runs the schema validator and Schemathesis fuzz locally, and wires the
> checks into the `verification` gate. No `delegate_task` needed for the core
> workflow; the checks are deterministic tool invocations.
