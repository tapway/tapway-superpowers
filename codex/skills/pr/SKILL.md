...
Read the project's integration branch from `AGENTS.md`:
```bash
TARGET=$(grep "^TARGET_BRANCH:" AGENTS.md 2>/dev/null | awk '{print $2}')
TARGET=${TARGET:-staging}   # default to staging if not set
echo "Targeting: $TARGET"
```

If `AGENTS.md` has no `TARGET_BRANCH` entry, default to `staging`. Add it once to avoid the lookup:
```
...
Ask Codex for help with specific conflicts:
```
resolve the conflict in src/services/auth.py — preserve the rate limiting from staging and the new JWT logic from this branch
```
...
the database, ask Codex to delegate to a subagent for the diff review:

```text
Ask Codex to delegate to a subagent to run the db-reviewer over this branch's diff.
...
### Step 5b: Sync Living-Docs to gbrain (CodeMAX — optional)

**Optional.** Only run this if your team uses CodeMAX/gbrain. If you don't, or
`CODEMAX_ENABLED` is not set, skip straight to Step 6 — nothing breaks.

If this PR touched product requirements, blueprints, work orders, or ADRs
(Layer B living-docs) and you use CodeMAX, sync them into gbrain so the brain
stays current. See the `codemax-gbrain` skill.

```bash
# Only if CODEMAX_ENABLED=1 and codemax is installed
...
**Skip this step** if: you don't use CodeMAX, the PR made no living-doc changes
(pure code), or `codemax` is unavailable. It is never required for the PR to
proceed.
...
```bash
# Read target branch
TARGET=$(grep "^TARGET_BRANCH:" AGENTS.md 2>/dev/null | awk '{print $2}'); TARGET=${TARGET:-staging}
