...
metadata:
  hermes:
    tags: [brainstorming, design, architecture, planning]
    related_skills: [interview, writing-plans, deep-research]
---
...
If the problem involves technology, services, or patterns your team hasn't used before, use Hermes web tools (`web_search` / `web_extract`) first to gather grounded options instead of guesses. Skip this step if the domain is familiar.
...
Before generating ANY option, retrieve the platform-wide design context. This step is enforced — `codemax platform-context check` will fail the document (and the Hermes pre-tool hook will block the write) if options exist without grounding.
