...
**Step 3 — Open Codex and start work**:
```bash
codex
```
Then tell Codex:
...
```
Codex will read the plan and checklist and begin `$tdd` automatically.

**Step 4 — When all your tasks are done**:
```
$cleanup    ← only if using the tapway template (removes scaffold placeholders)
